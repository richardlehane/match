...
func BenchmarkNew(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = newWac([]Seq{seq("handle"), seq("handl"), seq("hand"), seq("han"), seq("ha"), seq("a")})
	}
}

func BenchmarkIndex(b *testing.B) {
	b.StopTimer()
	ac := newWac([]Seq{seq("handle"), seq("handl"), seq("hand"), seq("han"), seq("ha"), seq("a")})
...
func BenchmarkMatchingNoMatch(b *testing.B) {
	...
	ac := newWac([]Seq{seq("abababababababd"),
		seq("abababb"),
		seq("abababababq")})
	b.StartTimer()
	for i := 0; i < b.N; i++ {
